...
public class VcfSettingsPage extends PageInfra {
	...
	public void addSeedSwitch(String name, String usrname, String pwd) throws Exception {
		vcfSettingsIcon.click();
	...
	public boolean isCollectorConfigured(String collName) {
		...
		// List<WebElement> collCount =
		// driver.findElements(By.cssSelector(collectorListId));
			...
			for (WebElement coll : collList) {
				if (coll.getText().contains(collName)) {
					...
					if (currentState == false) {
						...
						return true; // No need to edit since the collector is
										// already in running state. Edit will
										// fail at this point.
					}
				}
			}
	...
	public boolean toggleCollState(String collName, boolean expState) throws Exception {
		...
		if (exists) {
			...
			for (WebElement coll : collList) {
				if (coll.getText().contains(collName)) {
					currentState = checkCollectorState(coll); // findCurrentState
																// of the switch
					if (currentState != expState) {
						...
						Thread.sleep(5000); // waiting for the toggle to go
											// through
						waitForElementVisibility(collectorList, 100);
						waitForElementToClick(By.cssSelector(collectorListId), 100);
					}
					if (expState == checkCollectorState(coll))
						status = true;
				}
				break;
			}
	...
	public boolean verifySeedSwitch(String name, String usrname, String pwd) {
		boolean status = false;
	...
	public boolean activateLicense(String usrname, String pwd, LicenseTypes type) throws Exception {
		...
		if (status == false) {
			com.jcabi.log.Logger.error("activateLicense",
					"Couldn't find license" + type.toString() + " for activation");
		} else {
			closePopUp();
		}
		return status;
